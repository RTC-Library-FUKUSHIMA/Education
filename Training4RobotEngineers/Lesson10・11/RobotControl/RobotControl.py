...
robotcontrol_spec = ["implementation_id", "RobotControl", 
		 "type_name",         "RobotControl", 
		 "description",       "ロボット制御コンポーネント", 
		 "version",           "1.0.0", 
		 "vendor",            "FSK", 
		 "category",          "Controller", 
...
# 
# 
class RobotControl(OpenRTM_aist.DataFlowComponentBase):
...
def RobotControlInit(manager):
    profile = OpenRTM_aist.Properties(defaults_str=robotcontrol_spec)
    manager.registerFactory(profile,
                            RobotControl,
                            OpenRTM_aist.Delete)

def MyModuleInit(manager):
    RobotControlInit(manager)

    # Create a component
    comp = manager.createComponent("RobotControl")
